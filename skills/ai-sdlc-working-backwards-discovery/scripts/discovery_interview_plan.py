#!/usr/bin/env python3
"""Compress discovery notes into working-backwards interview gaps."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"Discovery Interview Gaps: {args.feature}",
        files=args.files,
        required_sections=["Customer", "Problem", "Value Proposition", "MVP", "Risks", "Success Metrics"],
        keywords=["customer", "pain", "metric", "mvp", "risk", "launch", "adoption"],
        prompts=["Ask only for missing customer/problem/value evidence in quick flow.", "Use full flow when MVP, risks, or success metrics are materially unclear."],
        ...
        write=args.write,
        skill_name="ai-sdlc-working-backwards-discovery",
        state_args=args,
    )
