#!/usr/bin/env python3
"""Compress discovery input into PRFAQ package readiness signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"PRFAQ Package Signals: {args.feature}",
        files=args.files,
        required_sections=["Press Release", "Customer FAQ", "Internal FAQ", "Business Requirements", "Risks", "Success Metrics"],
        keywords=["customer", "faq", "launch", "benefit", "requirement", "risk", "metric"],
        prompts=["Draft PRFAQ only from confirmed discovery facts.", "Move unresolved launch or value choices into decision-log.md."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="prfaq.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-prfaq-package-synthesis",
        state_args=args,
    )
