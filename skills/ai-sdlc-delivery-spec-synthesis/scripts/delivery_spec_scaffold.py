#!/usr/bin/env python3
"""Compress refined artifacts into delivery-spec synthesis signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"Delivery Spec Synthesis Signals: {args.feature}",
        files=args.files,
        required_sections=["Summary", "Scope", "Requirements", "User Stories", "Acceptance", "Dependencies", "QA Notes", "Handoff Risks"],
        keywords=["requirement", "story", "acceptance", "dependency", "risk", "qa", "handoff"],
        prompts=["Promote clarified delivery context into engineering-ready requirements.", "Keep unresolved delivery choices in decision-log.md."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="delivery-spec.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-delivery-spec-synthesis",
        state_args=args,
    )
