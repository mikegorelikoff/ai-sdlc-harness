#!/usr/bin/env python3
"""Compress backlog artifacts into release-slicing readiness signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"Release Slicing Signals: {args.feature}",
        files=args.files,
        required_sections=["MVP", "Release Slices", "Priorities", "Dependencies", "Readiness", "Risks"],
        keywords=["mvp", "release", "slice", "priority", "dependency", "risk", "blocked"],
        prompts=["Keep MVP limited to independently valuable stories.", "Flag slices with unresolved dependencies or decisions."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="release-slicing.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-release-slicing-and-backlog-readiness-review",
        state_args=args,
    )
