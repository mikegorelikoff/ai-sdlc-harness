...
| DEC-005 | 2026-07-21 | accepted | Security and repository maintainers | Treat live marketplace findings as revision-specific scanner evidence; fix confirmed boundaries in source and require a post-publication provider rescan before claiming the external warnings are cleared. | Skills.sh reports 11 flagged skills across Gen Agent Trust Hub, Socket, and Snyk, while the repository can only change a newer candidate revision. | Ignore scanner findings; promise immediate external clearance; remediate source, retain all dispositions, and label rescan lag. | affected SKILL.md files; shared compatibility runtime; security-contract tests; marketplace audit report | AC-012; AC-016; TC-016 |
