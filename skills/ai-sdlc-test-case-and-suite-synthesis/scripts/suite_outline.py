#!/usr/bin/env python3
"""Compress test cases into suite synthesis signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"Test Suite Synthesis Signals: {args.feature}",
        files=args.files,
        required_sections=["Smoke Suite", "Regression Suite", "UAT Suite", "Entry Criteria", "Exit Criteria"],
        keywords=["smoke", "regression", "uat", "suite", "entry", "exit", "priority"],
        prompts=["Put only critical path tests in smoke.", "Keep regression suite risk-based and traceable to requirements."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="test-suites.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-test-case-and-suite-synthesis",
        state_args=args,
    )
