#!/usr/bin/env python3
"""Compress requirements into risk-based test strategy signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"Test Strategy Signals: {args.feature}",
        files=args.files,
        required_sections=["Scope", "Coverage Priorities", "Suite Intent", "Test Data", "Environment Dependencies", "Risks"],
        keywords=["risk", "coverage", "suite", "data", "environment", "integration", "regression"],
        prompts=["Map each high-risk behavior to a coverage layer.", "Keep suite intent separate from individual test cases."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="test-strategy.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-test-scope-and-strategy-design",
        state_args=args,
    )
