#!/usr/bin/env python3
"""Compress planning artifacts into goal-capability-epic mapping signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"Goal Capability Mapping Signals: {args.feature}",
        files=args.files,
        required_sections=["Goals", "Capabilities", "Actors", "Epics", "Outcome Metrics"],
        keywords=["goal", "capability", "epic", "actor", "outcome", "metric", "priority"],
        prompts=["Map each capability to one measurable goal.", "Flag epics without actors or outcome metrics."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="goal-capability-epic-map.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-goal-capability-and-epic-mapping",
        state_args=args,
    )
