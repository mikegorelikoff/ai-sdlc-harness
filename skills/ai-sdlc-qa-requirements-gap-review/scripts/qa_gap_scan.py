#!/usr/bin/env python3
"""Compress requirements into QA-blocking gap signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"QA Requirements Gap Signals: {args.feature}",
        files=args.files,
        required_sections=["Acceptance Criteria", "Negative Cases", "Test Data", "Environment", "Non-Functional Requirements", "Open Questions"],
        keywords=["testable", "negative", "edge", "data", "environment", "performance", "security"],
        prompts=["Flag requirements that cannot produce observable pass/fail tests.", "Separate QA blockers from normal test design work."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="qa-requirements-gap-review.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-qa-requirements-gap-review",
        state_args=args,
    )
