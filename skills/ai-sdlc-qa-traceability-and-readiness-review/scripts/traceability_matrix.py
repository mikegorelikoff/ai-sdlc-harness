#!/usr/bin/env python3
"""Compress requirements and tests into traceability readiness signals."""
...
def main() -> int:
    ...
    return emit_profile_report(
        title=f"QA Traceability Signals: {args.feature}",
        files=args.files,
        required_sections=["Requirements", "Acceptance Criteria", "Test Cases", "Suites", "Coverage Gaps", "Readiness"],
        keywords=["req-", "ac-", "tc-", "coverage", "gap", "ready", "blocked"],
        prompts=["Flag requirements without test coverage.", "Report readiness only after gaps are classified by severity."],
        summary_limit=args.summary_limit,
        flow_mode=flow_mode(args),
        feature=args.feature,
        artifact_name="qa-traceability.md",
        workspace="refinement",
        emit_template=args.emit_template,
        emit_decision_log_entry=args.emit_decision_log_entry,
        write=args.write,
        skill_name="ai-sdlc-qa-traceability-and-readiness-review",
        state_args=args,
    )
